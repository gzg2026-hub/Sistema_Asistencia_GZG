...
if not df_trab.empty and not df_marc.empty:
    ...
    out_path = os.path.join(ROOT_DIR, "Reporte_Asistencia_Procesado.xlsx")
    try:
        with open(out_path, "wb") as f:
            f.write(excel_bytes)
        print(f"[OK] Archivo actualizado exitosamente en: {out_path}")
    except PermissionError:
        alt_path = os.path.join(ROOT_DIR, "Reporte_Asistencia_Procesado_Nuevo.xlsx")
        with open(alt_path, "wb") as f:
            f.write(excel_bytes)
        print(f"[OK] Archivo bloqueado por Excel. Guardado exitosamente como: {alt_path}")
